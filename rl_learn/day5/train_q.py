...
BEHAVIOR    = "uniform"   # "epsilon_greedy" (跟 Q 走 ε) 或 "uniform" (永远 50/50 完全随机)
                          # off-policy 的 Q-learning 理论上不依赖 behavior, 可以试纯 uniform
...
def epsilon_greedy(Q, box, eps, rng):
    """根据 BEHAVIOR 选 behavior policy. uniform 模式下完全忽略 Q 和 eps."""
    if BEHAVIOR == "uniform":
        return int(rng.integers(0, N_ACTIONS))    # 永远 50/50 完全随机
    if rng.random() < eps:
        return int(rng.integers(0, N_ACTIONS))
    return int(np.argmax(Q[box]))

claude
def run_episode_q(env, Q, N, eps, alpha, gamma, rng, seed=None):
    ...
    for t in range(1000):
        ...
        if done:
            ...
            target = r + gamma * Q[next_box].max()      # off-policy: max 替代 sample
        N[box, a] += 1
        step = (1.0 / N[box, a]) if alpha is None else alpha
        Q[box, a] += step * (target - Q[box, a])
        if done:
            break
        next_a = epsilon_greedy(Q, next_box, eps, rng)  # behavior 仍然 ε-greedy, 用于 step 下一动作
...
def train(env, n_episodes, gamma, eps_start, eps_min, decay_episodes, rng):
    ...
    for ep in range(n_episodes):
        ...
        G_0 = run_episode_q(env, Q, N, eps=eps, alpha=ALPHA, gamma=gamma, rng=rng, seed=ep)
...
def main():
    ...
    print(f"=== Q-learning 训练 CartPole ===")
    ...
    if BEHAVIOR != "epsilon_greedy":
        tag = f"{tag}_{BEHAVIOR}"   # 加 behavior 后缀, 不覆盖 ε-greedy 版本
    plot_path = os.path.join(figs_dir, f"q_{tag}_training.png")
    plot_training(G_hist, eps_hist, cov_hist, plot_path)
    print(f"\n  学习曲线 → {plot_path}")

    Q_path = os.path.join(os.path.dirname(__file__), f"trained_Q_q_{tag}.npy")
    np.save(Q_path, Q)
    N_path = os.path.join(os.path.dirname(__file__), f"trained_N_q_{tag}.npy")
    np.save(N_path, N)
